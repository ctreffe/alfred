UnlinkedDataPage
=============================

.. currentmodule:: alfred3.page

.. autoclass:: UnlinkedDataPage
   :show-inheritance:


   
   
   .. rubric:: Methods

   .. autosummary::
      :nosignatures:
      :toctree:
      
   
   
   
      ~UnlinkedDataPage.added_to_experiment
   
   
      ~UnlinkedDataPage.added_to_section
   
   
      ~UnlinkedDataPage.append
   
   
      ~UnlinkedDataPage.close
   
   
      ~UnlinkedDataPage.custom_move
   
   
      ~UnlinkedDataPage.durations
   
   
      ~UnlinkedDataPage.first_duration
   
   
      ~UnlinkedDataPage.last_duration
   
   
      ~UnlinkedDataPage.on_close
   
   
      ~UnlinkedDataPage.on_each_hide
   
   
      ~UnlinkedDataPage.on_each_show
   
   
      ~UnlinkedDataPage.on_exp_access
   
   
      ~UnlinkedDataPage.on_first_hide
   
   
      ~UnlinkedDataPage.on_first_show
   
   
      ~UnlinkedDataPage.position_in_section
   
   
      ~UnlinkedDataPage.prepare_web_widget
   
   
      ~UnlinkedDataPage.save_data
   
   
      ~UnlinkedDataPage.showif
   
   
      ~UnlinkedDataPage.uptree
   
   
      ~UnlinkedDataPage.validate
   
   
      ~UnlinkedDataPage.visible
   

   
   

   
   
   .. rubric:: Attributes

   .. autosummary::
      :template: autosummary/attribute.rst
      :toctree:

      
         ~UnlinkedDataPage.all_elements
         ~UnlinkedDataPage.all_input_elements
         ~UnlinkedDataPage.background_color
         ~UnlinkedDataPage.data
         ~UnlinkedDataPage.elements
         ~UnlinkedDataPage.encrypt
         ~UnlinkedDataPage.exp
         ~UnlinkedDataPage.experiment
         ~UnlinkedDataPage.filled_input_elements
         ~UnlinkedDataPage.fixed_width
         ~UnlinkedDataPage.has_been_shown
         ~UnlinkedDataPage.header_color
         ~UnlinkedDataPage.input_elements
         ~UnlinkedDataPage.instance_log
         ~UnlinkedDataPage.is_closed
         ~UnlinkedDataPage.minimum_display_time
         ~UnlinkedDataPage.minimum_display_time_msg
         ~UnlinkedDataPage.must_be_shown
         ~UnlinkedDataPage.name
         ~UnlinkedDataPage.parent
         ~UnlinkedDataPage.parent_name
         ~UnlinkedDataPage.prefix_element_names
         ~UnlinkedDataPage.progress
         ~UnlinkedDataPage.responsive_width
         ~UnlinkedDataPage.section
         ~UnlinkedDataPage.short_tree
         ~UnlinkedDataPage.should_be_shown
         ~UnlinkedDataPage.subtitle
         ~UnlinkedDataPage.tag
         ~UnlinkedDataPage.title
         ~UnlinkedDataPage.tree
         ~UnlinkedDataPage.uid
         ~UnlinkedDataPage.unlinked_data
         ~UnlinkedDataPage.updated_elements
         ~UnlinkedDataPage.vargs
      
   
   
   