SingleChoice
==================================

.. currentmodule:: alfred3.element.input

.. autoclass:: SingleChoice
   :show-inheritance:




   .. rubric:: Methods

   .. autosummary::
      :nosignatures:
      :toctree:




      ~SingleChoice.add_css


      ~SingleChoice.add_js


      ~SingleChoice.added_to_experiment


      ~SingleChoice.added_to_page


      ~SingleChoice.define_choices


      ~SingleChoice.prepare_web_widget


      ~SingleChoice.render_inner_html


      ~SingleChoice.set_data


      ~SingleChoice.validate_data







   .. rubric:: Attributes

   .. autosummary::
      :template: autosummary/attribute.rst
      :toctree:


         ~SingleChoice.base_template
         ~SingleChoice.bottomlab
         ~SingleChoice.choice_labels
         ~SingleChoice.codebook_data
         ~SingleChoice.converted_width
         ~SingleChoice.corrective_hints
         ~SingleChoice.css_class_container
         ~SingleChoice.css_class_element
         ~SingleChoice.css_code
         ~SingleChoice.css_urls
         ~SingleChoice.data
         ~SingleChoice.debug_enabled
         ~SingleChoice.debug_value
         ~SingleChoice.default
         ~SingleChoice.default_no_input_hint
         ~SingleChoice.description
         ~SingleChoice.disabled
         ~SingleChoice.display_standalone
         ~SingleChoice.element_template
         ~SingleChoice.element_width
         ~SingleChoice.emojize
         ~SingleChoice.exp
         ~SingleChoice.experiment
         ~SingleChoice.font_size
         ~SingleChoice.force_input
         ~SingleChoice.hint_manager
         ~SingleChoice.input
         ~SingleChoice.js_code
         ~SingleChoice.js_urls
         ~SingleChoice.labels
         ~SingleChoice.layout
         ~SingleChoice.leftlab
         ~SingleChoice.name
         ~SingleChoice.no_input_hint
         ~SingleChoice.page
         ~SingleChoice.position
         ~SingleChoice.prefix
         ~SingleChoice.rightlab
         ~SingleChoice.section
         ~SingleChoice.short_tree
         ~SingleChoice.should_be_shown
         ~SingleChoice.show_hints
         ~SingleChoice.showif
         ~SingleChoice.suffix
         ~SingleChoice.template_data
         ~SingleChoice.toplab
         ~SingleChoice.tree
         ~SingleChoice.type
         ~SingleChoice.vertical
         ~SingleChoice.web_widget
         ~SingleChoice.width
