SelectPageList
====================================

.. currentmodule:: alfred3.element.input

.. autoclass:: SelectPageList
   :show-inheritance:




   .. rubric:: Methods

   .. autosummary::
      :nosignatures:
      :toctree:




      ~SelectPageList.add_css


      ~SelectPageList.add_js


      ~SelectPageList.added_to_experiment


      ~SelectPageList.added_to_page


      ~SelectPageList.define_choices


      ~SelectPageList.prepare_web_widget


      ~SelectPageList.render_inner_html


      ~SelectPageList.set_data


      ~SelectPageList.validate_data







   .. rubric:: Attributes

   .. autosummary::
      :template: autosummary/attribute.rst
      :toctree:


         ~SelectPageList.base_template
         ~SelectPageList.bottomlab
         ~SelectPageList.choice_labels
         ~SelectPageList.codebook_data
         ~SelectPageList.converted_width
         ~SelectPageList.corrective_hints
         ~SelectPageList.css_class_container
         ~SelectPageList.css_class_element
         ~SelectPageList.css_code
         ~SelectPageList.css_urls
         ~SelectPageList.data
         ~SelectPageList.debug_enabled
         ~SelectPageList.debug_value
         ~SelectPageList.default
         ~SelectPageList.default_no_input_hint
         ~SelectPageList.description
         ~SelectPageList.disabled
         ~SelectPageList.display_standalone
         ~SelectPageList.element_template
         ~SelectPageList.element_width
         ~SelectPageList.emojize
         ~SelectPageList.exp
         ~SelectPageList.experiment
         ~SelectPageList.font_size
         ~SelectPageList.force_input
         ~SelectPageList.hint_manager
         ~SelectPageList.input
         ~SelectPageList.js_code
         ~SelectPageList.js_urls
         ~SelectPageList.labels
         ~SelectPageList.layout
         ~SelectPageList.leftlab
         ~SelectPageList.name
         ~SelectPageList.no_input_hint
         ~SelectPageList.page
         ~SelectPageList.position
         ~SelectPageList.prefix
         ~SelectPageList.rightlab
         ~SelectPageList.section
         ~SelectPageList.short_tree
         ~SelectPageList.should_be_shown
         ~SelectPageList.show_hints
         ~SelectPageList.showif
         ~SelectPageList.suffix
         ~SelectPageList.template_data
         ~SelectPageList.toplab
         ~SelectPageList.tree
         ~SelectPageList.type
         ~SelectPageList.vertical
         ~SelectPageList.web_widget
         ~SelectPageList.width
