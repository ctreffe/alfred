MultipleChoice
====================================

.. currentmodule:: alfred3.element.input

.. autoclass:: MultipleChoice
   :show-inheritance:


   
   
   .. rubric:: Methods

   .. autosummary::
      :nosignatures:
      :toctree:
      
   
   
   
      ~MultipleChoice.add_css
   
   
      ~MultipleChoice.add_js
   
   
      ~MultipleChoice.added_to_experiment
   
   
      ~MultipleChoice.added_to_page
   
   
      ~MultipleChoice.define_choices
   
   
      ~MultipleChoice.prepare_web_widget
   
   
      ~MultipleChoice.render_inner_html
   
   
      ~MultipleChoice.set_data
   
   
      ~MultipleChoice.validate_data
   

   
   

   
   
   .. rubric:: Attributes

   .. autosummary::
      :template: autosummary/attribute.rst
      :toctree:

      
         ~MultipleChoice.base_template
         ~MultipleChoice.bottomlab
         ~MultipleChoice.choice_labels
         ~MultipleChoice.codebook_data
         ~MultipleChoice.converted_width
         ~MultipleChoice.corrective_hints
         ~MultipleChoice.css_class_container
         ~MultipleChoice.css_class_element
         ~MultipleChoice.css_code
         ~MultipleChoice.css_urls
         ~MultipleChoice.data
         ~MultipleChoice.debug_enabled
         ~MultipleChoice.debug_value
         ~MultipleChoice.default
         ~MultipleChoice.default_no_input_hint
         ~MultipleChoice.description
         ~MultipleChoice.disabled
         ~MultipleChoice.display_standalone
         ~MultipleChoice.element_template
         ~MultipleChoice.element_width
         ~MultipleChoice.emojize
         ~MultipleChoice.exp
         ~MultipleChoice.experiment
         ~MultipleChoice.font_size
         ~MultipleChoice.force_input
         ~MultipleChoice.hint_manager
         ~MultipleChoice.input
         ~MultipleChoice.js_code
         ~MultipleChoice.js_urls
         ~MultipleChoice.labels
         ~MultipleChoice.layout
         ~MultipleChoice.leftlab
         ~MultipleChoice.name
         ~MultipleChoice.no_input_hint
         ~MultipleChoice.page
         ~MultipleChoice.position
         ~MultipleChoice.prefix
         ~MultipleChoice.rightlab
         ~MultipleChoice.section
         ~MultipleChoice.select_hint
         ~MultipleChoice.short_tree
         ~MultipleChoice.should_be_shown
         ~MultipleChoice.show_hints
         ~MultipleChoice.showif
         ~MultipleChoice.suffix
         ~MultipleChoice.template_data
         ~MultipleChoice.toplab
         ~MultipleChoice.tree
         ~MultipleChoice.type
         ~MultipleChoice.vertical
         ~MultipleChoice.web_widget
         ~MultipleChoice.width
      
   
   
   