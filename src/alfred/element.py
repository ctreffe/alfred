...
class Element(object):
    ...
    **Description:** Baseclass for every element with basic arguments.
    ...
    def should_be_shown(self):
        ...
        """
        return self._should_be_shown and self._should_be_shown_filter_function(self._page._experiment)
...
class PasswordElement(TextEntryElement):
    ...
    def wrong_password_hint(self):
        ...
        elif self._page and self._page._experiment\
...
class WebAudioElement(Element, WebElementInterface):
    ...
    def prepare_web_widget(self):
        ...
        if self._wav_audio_url is None and self._wav_path is not None:
            self._wav_audio_url = self._page._experiment.user_interface_controller.add_static_file(self._wav_path)

        if self._ogg_audio_url is None and self._ogg_path is not None:
            self._ogg_audio_url = self._page._experiment.user_interface_controller.add_static_file(self._ogg_path)

        if self._mp3_audio_url is None and self._mp3_path is not None:
            self._mp3_audio_url = self._page._experiment.user_interface_controller.add_static_file(self._mp3_path)
