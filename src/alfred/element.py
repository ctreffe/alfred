...
class ProgressBar(Element, WebElementInterface):
    ...
    def web_widget(self):
        ...
        widget = '<div class="progress-bar"><table class="%s" style="font-size: %spt;">' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size))

        widget = widget + '<tr><td><table class="%s"><tr><td style="%s %s">%s</td>' % (alignment_converter(self._alignment, 'container'), 'width: %spx;' % self._instruction_width if self._instruction_width is not None else "", 'height: %spx;' % self._instruction_height if self._instruction_height is not None else "", self._instruction)

        widget = widget + '<td><meter value="%s" min="%s" max="%s" style="font-size: %spt; width: %spx; margin-left: 5px;"></meter></td>' % (self._bar_value, self._bar_range[0], self._bar_range[1], fontsize_converter(self._font_size) + 5, self._bar_width if self._bar_width is not None else '200')

        widget = widget + '<td style="font-size: %spt; padding-left: 5px;">%s</td>' % (fontsize_converter(self._font_size), str(int(old_div(self._bar_value, (self._bar_range[1] - self._bar_range[0]) * 100))) + '%')
...
class TextAreaElement(TextEntryElement):
    ...
    def web_widget(self):
        ...
        widget = '<div class="text-area-element"><table class="%s" style="font-size: %spt;">' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size))

        widget = widget + '<tr><td class="itempagination-left" style="padding-bottom: 10px;">%s</td></tr>' % (self._instruction)

        widget = widget + '<tr><td class="%s"><textarea class="text-input pagination-left" style="font-size: %spt; height: %spx; width: %spx;" name="%s" %s>%s</textarea></td></tr>' % (alignment_converter(self._alignment), fontsize_converter(self._font_size), self._y_size, self._x_size, self.name, "" if self.enabled else " disabled=\"disabled\"", self._input)

        if self.corrective_hints:
            widget = widget + '<tr><td class="corrective-hint %s" style="font-size: %spt;">%s</td></tr>' % (alignment_converter(self._alignment, 'both'), fontsize_converter(self._font_size) - 1, self.corrective_hints[0])
...
class PasswordElement(TextEntryElement):
    ...
    def web_widget(self):
        ...
        widget = '<div class="text-entry-element"><table class="%s" style="font-size: %spt;">' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size))

        widget = widget + '<tr><td valign="bottom"><table class="%s"><tr><td %s>%s</td>' % (alignment_converter(self._alignment, 'container'), 'style="width: %spx;"' % self._instruction_width if self._instruction_width is not None else "", self._instruction)

        widget = widget + '<td valign="bottom"><input class="text-input" type="password" style="font-size: %spt; margin-bottom: 0px; margin-left: 5px;" name="%s" value="%s" %s /></td></tr></table></td></tr>' % (fontsize_converter(self._font_size), self.name, self._input, "" if self.enabled else 'disabled="disabled"')

        if self.corrective_hints:
            widget = widget + '<tr><td><table class="corrective-hint containerpagination-right"><tr><td style="font-size: %spt;">%s</td></tr></table></td></tr>' % (fontsize_converter(self._font_size), self.corrective_hints[0])
...
class LikertMatrix(InputElement, WebElementInterface):
    ...
    def data(self):
        ...
        for i in range(self._items):
            ...
            lm_data.update({label: None if int(self._input[i]) + 1 == 0 else int(self._input[i]) + 1})
    ...
    def web_widget(self):
        ...
        widget = '<div class="likert-matrix"><table class="%s" style="clear: both; font-size: %spt; margin-bottom: 10px;"><tr><td %s>%s</td></tr></table>' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size), 'style="width: %spx;"' % self._instruction_width if self._instruction_width is not None else "", self._instruction)  # Extra Table for instruction

        widget = widget + '<table class="%s %s table" style="width: auto; clear: both; font-size: %spt; margin-bottom: 10px;">' % (alignment_converter(self._alignment, 'container'), 'table-striped' if self._table_striped else '', fontsize_converter(self._font_size))  # Beginning Table
        ...
        if not self._transpose:
            ...
            for i in self._permutation:
                ...
                if self._item_labels:
                    widget = widget + '<td style="text-align:right; vertical-align: middle;">%s</td>' % self._item_labels[i * 2]  # Adding left itemlabel
                else:
                ...
                if self._item_labels:
                    widget = widget + '<td style="text-align:left;vertical-align: middle;">%s</td>' % self._item_labels[(i + 1) * 2 - 1]  # Adding right itemlabel
                else:
                    widget = widget + '<td></td>'  # Placeholder if no item_labels set
            ...
            if self._item_labels:
                ...
                for i in range(old_div(len(self._item_labels), 2)):
                    widget = widget + '<td class="pagination-centered containerpagination-centered" style="text-align:center; vertical-align: bottom;">%s</td>' % self._item_labels[i * 2]  # Adding top item_labels
            ...
            for i in range(self._levels):
                ...
                if self._top_scale_labels:
                    widget = widget + '<td class="pagination-right" style="vertical-align: middle;">%s</td>' % self._top_scale_labels[i]  # Adding left scalelabel
                else:
                ...
                if self._bottom_scale_labels:
                    widget = widget + '<td class="pagination-left" style="vertical-align: middle;">%s</td>' % self._bottom_scale_labels[i]  # Adding right scalelabel
                else:
                    widget = widget + '<td></td>'  # Placeholder if no scalelabels set
            ...
            if self._item_labels:
                ...
                for i in range(old_div(len(self._item_labels), 2)):
                    widget = widget + '<td class="pagination-centered containerpagination-centered" style="text-align:center; vertical-align: top;">%s</td>' % self._item_labels[(i + 1) * 2 - 1]  # Adding bottom item_labels
        ...
        if self.corrective_hints:
            ...
            widget = widget + '<table class="%s" style="clear: both; font-size: %spt;"><tr><td class="corrective-hint" >%s</td></tr></table>' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size) - 1, self.corrective_hints[0])
...
class LikertElement(LikertMatrix):
    ...
    def data(self):
        lm_data = {}
        lm_data.update({self.name: None if int(self._input[0]) + 1 == 0 else int(self._input[0]) + 1})
        return lm_data
...
class SingleChoiceElement(LikertElement):
    ...
    def web_widget(self):
        ...
        for i in range(self._items):  # Adding Radiobuttons for each sclae level in each likert item
            ...
            widget = widget + '<td class="pagination-left" style="vertical-align: middle;" %s>%s</td></tr>' % ('width: ' + str(self._item_label_width) + 'px;' if self._item_label_width else '', self._item_labels[self._permutation[i]])  # Adding item label

        widget = widget + "</table>"  # Closing table for SingleChoiceElement

        if self.corrective_hints:
            widget = widget + '<table class="%s" style="clear: both; font-size: %spt;"><tr><td class="corrective-hint" >%s</td></tr></table>' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size), self.corrective_hints[0])
...
class MultipleChoiceElement(LikertElement):
        ...
        **SingleChoiceElement** returns a vertically layouted item with adjustable choice alternatives (comparable to levels of likert scale)
        ...
        if select_hint:
            ...
            if min_select and not max_select:
                self._select_hint = u"Bitte wählen Sie mindestens %i Optionen aus" % self._min_select
            elif max_select and not min_select:
                self._select_hint = u"Bitte wählen Sie höchstens %i Optionen aus" % self._max_select
            elif max_select and min_select:
                self._select_hint = u"Bitte wählen Sie mindestens %i und höchstens %i Optionen aus" % (self._min_select, self._max_select)
        ...
        if default is not None:
            ...
            if not len(self._input) == len(self._item_labels):
                raise ValueError('Wrong default data! Default value must be set to a list of %s values containing either "0" or "1"!' % (len(self._item_labels)))
    ...
    def web_widget(self):
        ...
        widget = widget + '<table class="%s %s" style="clear: both; font-size: %spt; line-height: normal; margin-bottom: 10px;">' % (alignment_converter(self._alignment, 'container'), 'table-striped' if self._table_striped else '', fontsize_converter(self._font_size))  # Beginning Table
        ...
        for i in range(self._items):
            ...
            widget = widget + '<td class="pagination-left" style="vertical-align: middle;">%s</td></tr>' % self._item_labels[self._permutation[i]]
...
class LikertListElement(InputElement, WebElementInterface):
    def __init__(self, instruction='', levels=7, top_scale_labels=None, bottom_scale_labels=None,
                 item_labels=[], item_label_height=None, item_label_width=None, item_label_alignment='left',
                 table_striped=False, spacing=30, shuffle=False, instruction_width=None,
                 instruction_height=None, use_short_labels=False, **kwargs):
        '''
...
class TableElement(Element, WebElementInterface):
    ...
    @property
    def web_widget(self):
        html = '<table class="%s" style="text-align: center; font-size:%spt">' % (alignment_converter(self._alignment, 'container'), fontsize_converter(self._font_size))
...
class WebAudioElement(Element, WebElementInterface):
    def __init__(self, wav_url=None, wav_path=None, ogg_url=None, ogg_path=None, mp3_url=None, mp3_path=None, controls=True, autoplay=False, loop=False, **kwargs):
        ...
        if self._wav_path is None and self._ogg_path is None and self._mp3_path is None and self._wav_audio_url is None and self._ogg_audio_url is None and self._mp3_audio_url is None:
            raise AlfredError

    def prepare_web_widget(self):

        if self._wav_audio_url is None and self._wav_path is not None:
            self._wav_audio_url = self._question._experiment.user_interface_controller.add_static_file(self._wav_path)

        if self._ogg_audio_url is None and self._ogg_path is not None:
            self._ogg_audio_url = self._question._experiment.user_interface_controller.add_static_file(self._ogg_path)

        if self._mp3_audio_url is None and self._mp3_path is not None:
            self._mp3_audio_url = self._question._experiment.user_interface_controller.add_static_file(self._mp3_path)
...
class WebVideoElement(Element, WebElementInterface):
    ...
    def prepare_web_widget(self):
        ...
        if self._mp4_video_url is None and self._mp4_path is not None:
            self._mp4_video_url = self._question._experiment.user_interface_controller.add_static_file(self._mp4_path)

        if self._ogg_video_url is None and self._ogg_path is not None:
            self._ogg_video_url = self._question._experiment.user_interface_controller.add_static_file(self._ogg_path)

        if self._web_m_video_url is None and self._web_m_path is not None:
            self._web_m_video_url = self._question._experiment.user_interface_controller.add_static_file(self._web_m_path)
