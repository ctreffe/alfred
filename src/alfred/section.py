...
class PageGroup(PageCore):
    ...
    def __str__(self):
        s = "PageGroup (tag = " + self.tag + ", questions:[" + str(self._page_list) + "]"
        return s
    ...
    def current_title(self):
        ...
        if isinstance(self._page_list[self._current_page_index], Page) and self._page_list[self._current_page_index].title is not None:
            return self._page_list[self._current_page_index].title

        return self.title

    @property
    def current_subtitle(self):
        if isinstance(self._page_list[self._current_page_index], PageGroup) and self._page_list[self._current_page_index].current_subtitle is not None:
            return self._page_list[self._current_page_index].current_subtitle

        if isinstance(self._page_list[self._current_page_index], Page) and self._page_list[self._current_page_index].subtitle is not None:
            return self._page_list[self._current_page_index].subtitle

        return self.subtitle

    @property
    def current_status_text(self):
        if isinstance(self._page_list[self._current_page_index], PageGroup) and self._page_list[self._current_page_index].current_status_text is not None:
            return self._page_list[self._current_page_index].current_status_text

        if isinstance(self._page_list[self._current_page_index], Page) and self._page_list[self._current_page_index].statustext is not None:
            return self._page_list[self._current_page_index].statustext
    ...
    def allow_leaving(self, direction):
        return self._page_list[self._current_page_index].allow_leaving(direction)
    ...
    def added_to_experiment(self, exp):
        ...
        for question in self._page_list:
    ...
    def can_move_backward(self):
        if isinstance(self._page_list[self._current_page_index], PageGroup) and self._page_list[self._current_page_index].can_move_backward:
            return True

        return reduce(lambda b, q_core: b or q_core.should_be_shown, self._page_list[:self._current_page_index], False)

    @property
    def can_move_forward(self):
        if isinstance(self._page_list[self._current_page_index], PageGroup) and self._page_list[self._current_page_index].can_move_forward:
            return True

        return reduce(lambda b, q_core: b or q_core.should_be_shown, self._page_list[self._current_page_index + 1:], False)
    ...
    def move_forward(self):
        ...
        if isinstance(self._page_list[self._current_page_index], PageGroup) \
                and self._page_list[self._current_page_index].can_move_forward:
            self._page_list[self._current_page_index].move_forward()

        else:
            # if current_question is QG: call leave
            if isinstance(self._core_question_at_index, PageGroup):
                self._core_question_at_index.leave(Direction.FORWARD)
            for index in range(self._current_page_index + 1, len(self._page_list)):
    ...
    def move_backward(self):
        ...
        if isinstance(self._page_list[self._current_page_index], PageGroup) \
                and self._page_list[self._current_page_index].can_move_backward:
            self._page_list[self._current_page_index].move_backward()

        else:
            # if current_question is QG: call leave
            if isinstance(self._core_question_at_index, PageGroup):
                self._core_question_at_index.leave(Direction.BACKWARD)
            for index in range(self._current_page_index - 1, -1, -1):
                if self._page_list[index].should_be_shown:
                    self._current_page_index = index
                    if isinstance(self._page_list[index], PageGroup):
    ...
    def move_to_last(self):
        ...
        if isinstance(self._core_question_at_index, PageGroup):
            self._core_question_at_index.leave(Direction.JUMP)
        self._current_page_index = len(self._page_list) - 1
        if self._page_list[self._current_page_index].should_be_shown:
            if isinstance(self._page_list[self._current_page_index], PageGroup):
                self._core_question_at_index.enter()
    ...
    def move_to_position(self, pos_list):
        ...
        self._current_page_index = pos_list[0]
        if isinstance(self._page_list[self._current_page_index], PageGroup):
            self._core_question_at_index.enter()
            if len(pos_list) == 1:
                self._page_list[self._current_page_index].move_to_first()
            else:
                self._page_list[self._current_page_index].move_to_position(pos_list[1:])

    @property
    def _core_question_at_index(self):
        return self._page_list[self._current_page_index]


class HeadOpenSection(PageGroup):
    def __init__(self, **kwargs):
        super(HeadOpenSection, self).__init__(**kwargs)
        self._max_page_index = 0

    @property
    def max_question_index(self):
        return self._max_page_index
    ...
    def move_forward(self):
        '''
        '''
        if self._max_page_index == self._current_page_index:
            if isinstance(self._core_question_at_index, Page):
                self._core_question_at_index.close_page()
        ...
        self._max_page_index = self._current_page_index

    def move_to_last(self):
        self._current_page_index = self._max_page_index

        if self._page_list[self._current_page_index].should_be_shown:
            if isinstance(self._page_list[self._current_page_index], PageGroup):
                self._page_list[self._current_page_index].move_to_last()
            return
        else:
            self.move_backward()
...
class SegmentedSection(HeadOpenSection):
    ...
    def move_to_position(self, pos_list):
        if self._current_page_index != pos_list[0]:
            raise MoveError()
    ...
    def jumplist(self):
        ...
        jumplist = []
        for item in super(HeadOpenSection, self).jumplist:
            if len(item[0]) == 0 or item[0][0] == self._current_page_index:
                jumplist.append(item)
