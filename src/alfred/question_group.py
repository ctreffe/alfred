...
class QuestionGroup(QuestionCore):
    ...
    def current_question(self):
        return self._questionList[self._currentQuestionIndex].current_question if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) else self._questionList[self._currentQuestionIndex]

    @property
    def current_title(self):
        if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) and self._questionList[self._currentQuestionIndex].current_title is not None:
    ...
    def current_subtitle(self):
        if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) and self._questionList[self._currentQuestionIndex].current_subtitle is not None:
    ...
    def current_status_text(self):
        if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) and self._questionList[self._currentQuestionIndex].current_status_text is not None:
    ...
    @QuestionCore.should_be_shown.getter
    def should_be_shown(self):
        ...
        return super(QuestionGroup, self).should_be_shown and reduce(lambda b, qCore: b or qCore.should_be_shown, self._questionList, False)
    ...
    def enter(self):
        ...
        if isinstance(self._core_question_at_index, QuestionGroup):
    ...
    def jumplist(self):
        ...
        for i in range(0, len(self._questionList)):
            ...
            elif isinstance(self._questionList[i], Question) and self._questionList[i].is_jumpable:
    ...
    def can_move_backward(self):
        if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) and self._questionList[self._currentQuestionIndex].can_move_backward:
            return True
    ...
    def can_move_forward(self):
        if isinstance(self._questionList[self._currentQuestionIndex], QuestionGroup) and self._questionList[self._currentQuestionIndex].can_move_forward:
            return True
...
class HeadOpenQG(QuestionGroup):
    ...
    def allow_leaving(self, direction):
        if direction != Direction.FORWARD:
            return super(HeadOpenQG, self).allow_leaving(direction)

        # direction is Direction.FORWARD

        if isinstance(self._core_question_at_index, Question):
            HeadOpenQG._set_show_corrective_hints(self._core_question_at_index, True)
            return self._core_question_at_index.allow_closing and super(HeadOpenQG, self).allow_leaving(direction)
        else:  # currentCoreQuestion is Group
            if not self._core_question_at_index.can_move_forward:
                HeadOpenQG._set_show_corrective_hints(self._core_question_at_index, True)
                return HeadOpenQG._allow_closing_all_child_questions(self._core_question_at_index) and super(HeadOpenQG, self).allow_leaving(direction)
            else:
                return super(HeadOpenQG, self).allow_leaving(direction)
    ...
    def can_move_forward(self):
        ...
        if isinstance(self._core_question_at_index, QuestionGroup) and \
                not self._core_question_at_index.can_move_forward and \
                not HeadOpenQG._allow_closing_all_child_questions(self._core_question_at_index):
            return True
        elif isinstance(self._core_question_at_index, Question) and \
                not self._core_question_at_index.allow_closing:
            return True
        else:
            return super(HeadOpenQG, self).can_move_forward
    ...
    def move_forward(self):
        ...
        if self._maxQuestionIndex == self._currentQuestionIndex:
            ...
            elif not self._core_question_at_index.can_move_forward:  # self._core_question_at_index is instance of QuestionGroup and at the last item
                if not HeadOpenQG._allow_closing_all_child_questions(self._core_question_at_index):
                    # TODO handle if not all questions are closable.
                    self._core_question_at_index.append_item(HeadOpenQGCantClose())

                else:  # all child question at current index allow closing
                    HeadOpenQG._close_child_questions(self._core_question_at_index)

        super(HeadOpenQG, self).move_forward()
    ...
    def _allow_closing_all_child_questions(questionGroup, L=None):
        ...
        for item in questionGroup._questionList:
            if isinstance(item, QuestionGroup):
                allow_closing = allow_closing and HeadOpenQG._allow_closing_all_child_questions(item, L)
            elif not item.allow_closing:  # item is instance of Question and does not allow closing
                allow_closing = False
                if L is not None:
                    L.append(item)
    ...
    def _close_child_questions(questionGroup):
        for item in questionGroup._questionList:
            if isinstance(item, Question):
                ...
                HeadOpenQG._close_child_questions(item)
    ...
    def _set_show_corrective_hints(coreQuestion, b):
        if isinstance(coreQuestion, Question):
            ...
            for item in questionGroup._questionList:
                HeadOpenQG._set_show_corrective_hints(item, b)
...
class SegmentedQG(HeadOpenQG):
    ...
    def move_to_position(self, posList):
        ...
        super(SegmentedQG, self).move_to_position(posList)
