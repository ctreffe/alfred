...
* In your script.py, you can now encrypt and decrypt data via `alfred.Experiment.decryptor`, which is an instance of `cryptography.fernet.Fernet`, initialized with a user-specific unique key fernet key generated by mortimer (**v0.4.4+**). It offers the methods `alfred.Experiment.decryptor.encrypt()` and `alfred.Experiment.decryptor.decrypt()`. **Note** that these methods require input of type `bytes` and return data of type `bytes`, so make sure to convert your data when using it. See the [Fernet documentation](https://cryptography.io/en/latest/fernet/) for more.
* For offline testing, `alfred.Experiment.decryptor` will be initialized with the key `OnLhaIRmTULrMCkimb0CrBASBc293EYCfdNuUvIohV8=`. **IMPORTANT**: This key is public. Encrypted data in local (e.g., offline) experiments is not safe. This functionality is provided exclusively for testing your experiment before uploading to mortimer and running.
